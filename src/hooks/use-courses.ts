...
export const useGetCourses = (params?: { search?: string }) => {
  const {
    data,
    isFetching: loading,
    error,
    ...
    queryFn: async () => {
      try {
        ...
        return {
          ...
          meta: res.data.meta || { total: 0, page: 1, pages: 1, limit: 10 }
        };
      } catch (err) {
        toast.error("Có lỗi xảy ra khi lấy dữ liệu khoá học");
        throw err;
      }
    },
    staleTime: 1000
  ...
  const {
    ...
    onSuccess: (newCourse) => {
      if (newCourse) {
        ...
        queryClient.setQueryData<Course[]>(["courses"], (oldData) => (oldData ? [...oldData, newCourse] : [newCourse]));
...
export const useDeleteCourse = (onSuccessCallback?: () => void) => {
  ...
  const {
    ...
    onSuccess: (_data, course) => {
      ...
      onSuccessCallback?.();
      queryClient.setQueryData<Course[]>(["courses"], (oldData) => oldData?.filter((item) => item.id !== course.id));
      queryClient.invalidateQueries({ queryKey: ["courses"] });
    },
    onError: (err) => {
